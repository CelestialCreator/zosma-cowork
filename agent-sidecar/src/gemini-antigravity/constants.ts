 ...
 *
 * ⚠️ This reuses Antigravity's embedded Google OAuth client + undocumented Code
 * Assist `v1internal` endpoints (reverse-engineered access from a non-official
 * client). It is ToS-gray and carries real account-ban risk — recommend a
 * SECONDARY Google account, never a primary. Everything here is overridable via
 * env so nothing is silently hard-pinned.
 *
 * Values mirror github.com/tuxevil/pi-antigravity-rotator (src/types.ts,
 * src/oauth.ts) — the client id/secret are the publicly-extractable Antigravity
 * desktop-app credentials, kept in their original base64 form.
 */
